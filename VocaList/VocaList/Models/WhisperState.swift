...
//        try startPlayback(url)
    ...
    func toggleRecord() async {
        if isRecording {
            ...
            // Check if we can transcribe before attempting to record
            guard canTranscribe else {
                print("Cannot transcribe - model may not be loaded")
                return
            }
            
            // Check if whisper context is available
            guard whisperContext != nil else {
                print("Whisper model not loaded yet")
                messageLog += "Whisper model not loaded yet\n"
                return
            }
                ...
                if granted {
                    Task {
                        do {
                            ...
                            self.messageLog += "Recording error: \(error.localizedDescription)\n"
                            self.isRecording = false
                        }
                    }
                } else {
                    print("Microphone permission denied")
                    self.messageLog += "Microphone permission denied\n"
                }
            }
        }
    }
    ...
    private func requestRecordPermission(response: @escaping @MainActor (Bool) -> Void) {
        ...
        if #available(iOS 17.0, *) {
            AVAudioApplication.requestRecordPermission { granted in
                Task { @MainActor in
                    print("iOS 17+ permission result: \(granted)")
                    response(granted)
                }
            }
        } else {
            AVAudioSession.sharedInstance().requestRecordPermission { granted in
                Task { @MainActor in
                    print("Legacy permission result: \(granted)")
                    response(granted)
                }
            }
        }
